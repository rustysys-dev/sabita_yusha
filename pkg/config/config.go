...
type Macro struct {
	...
	OnPress    *entity.Command         `yaml:"on_press,omitempty"`
	OnRelease  *entity.Command         `yaml:"on_release,omitempty"`
	OnHold     *entity.Command         `yaml:"on_hold,omitempty"`
	HandlerMap map[int]*entity.Command `yaml:"-"`
}
...
func (m *Macro) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	*m = Macro(aux)
	...
	m.HandlerMap = make(map[int]*entity.Command)
	if m.OnPress != nil {
	...
	if _, err := os.Stat(name); os.IsNotExist(err) {
		defaultConfig := &Config{
			...
			CustomMacro: []Macro{
				{
					Name: "KEY_MACRO1",
					OnPress: &entity.Command{
						Type: "bash",
						Runner: &entity.BashCommand{
							Entrypoint: "echo",
